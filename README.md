...
| implementation | 개요·구조·부채·검증·API | 무엇을 하는가 · 다음 할 일 |

초안은 Kind 와 Nature 를 그대로 가집니다. **초안은 다른 종류의 문서가 아니라 같은 문서의
이른 시점**이기 때문입니다. 문서 끝에는 다음에 채울 항목이 순서대로 적혀 있습니다.

### 다섯 단계로 키우기 (`mmodule grow`)

`grow` 는 한 번에 한 칸씩 올라갑니다. 초안에서 완전판으로 한 번에 뛰면, 초안 모드가
없애려던 "빈 표제의 벽"을 한 단계 뒤로 미루는 것에 지나지 않기 때문입니다.

| 단계 | 붙는 것 | 이제 답할 수 있는 것 |
|---|---|---|
| 1 씨앗 | (초안) | 지금 아는 것과 모르는 것 |
| 2 정체 | 기준·신뢰도 헤더 + 첫 절 | 이게 무엇이고 지금 얼마나 믿을 만한가 |
| 3 뼈대 | 본문과 성격별 세부 목차 | 무엇을 어디에 채울지 |
| 4 결론 | 남은 본문 절 | 무엇이 남았고 지금 결론은 무엇인가 |
| 5 완성 | 결정·의존성·인터페이스·범위 | 남이 인용하고 이어 쓸 수 있게 |

**네 칸이 `module` 과 `current` 안에서 움직입니다.** 모듈의 본체가 거기 있기 때문입니다.
나머지 네 파트는 참조용 뼈대이므로 마지막 한 칸에 함께 붙습니다.

앞은 촘촘하고 뒤는 성깁니다. 문서를 포기하는 지점이 앞쪽이라서 두 번째 칸이 싸야 하고,
네 번째 칸까지 왔다면 이미 쓸 만한 문서가 있어서 뼈대가 한꺼번에 붙어도 부담이 아니라
정리로 읽히기 때문입니다.

```bash
mmodule grow "asyncio"            # 한 칸 올라간다
mmodule grow "asyncio" --level 3  # 3단계까지
mmodule grow "asyncio" --all      # 한 번에 완전판
```
...
| Kind | 1 | 2 | 3 | 4 | 5 |
|---|---|---|---|---|---|
| knowledge/concept | 42 | 57 | 81 | 111 | 241 |
| implementation | 35 | 48 | 62 | 98 | 245 |
| intent/plan | 40 | 57 | 107 | 140 | 287 |

이미 쓴 내용은 건드리지 않습니다. 문서에 없는 절만 뒤에 덧붙이며, 판단 기준은 표제의
존재 여부입니다. 지금 몇 단계인지는 헤더의 `**Level:**` 필드가 알려 주고, 그 값은 파일
내용에서 다시 세므로 손으로 고쳐도 어긋나지 않습니다. Kind 와 Nature 는 문서 헤더에서
읽으므로 다시 지정할 필요가 없습니다.

`mmodule create` 를 거치지 않고 **손으로 쓴 문서도 키울 수 있습니다.** 파이썬이 없는
모바일에서 템플릿으로 초안을 쓰고, 동기화한 뒤 PC 에서 키우는 경로가 이것입니다.
조건은 두 가지입니다.

- **파일 이름이 폴더 이름과 같아야 합니다.** `modules/asyncio/asyncio.md` 는 모듈
  `asyncio` 이지만, `modules/asyncio/초안.md` 는 `asyncio/초안` 이라는 별개 모듈로
  인식됩니다. 이 실수는 `mmodule grow` 가 진단해 알려 줍니다.
- **`**Kind:**` 줄이 있어야 합니다.** 어느 골격으로 키울지 알아내는 유일한 근거입니다.
  `**Nature:**` 까지 있으면 본문 목차도 함께 채워집니다.
...
mmodule grow "asyncio"                             # 한 칸씩, 다섯 단계로
