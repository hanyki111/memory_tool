...
| `Ctrl+P` → `모듈 전체 골격 붙이기` | 🌱 초안에 없는 절 덧붙이기 (`mmodule grow`) |
...
`Ctrl+P` → **모듈 전체 골격 붙이기 (mmodule grow)** 를 실행하면, **지금 열어 둔 문서**를
대상으로 없는 절만 덧붙입니다. 씨앗이 좁다고 느끼는 순간이 곧 그 문서를 보고 있을
때이기 때문입니다. 열린 문서가 모듈이 아니면 모듈 목록을 띄웁니다.

이미 쓴 내용은 건드리지 않습니다. 이미 모든 절이 있으면 아무것도 쓰지 않고 그렇다고
알려 줍니다.
...
새 기능이 보이지 않을 때는 대개 **낡은 `main.js` 를 복사한 것**입니다. 플러그인 목록의
**Memory Tool Integration** 아래에 표시되는 버전과 `manifest.json` 의 버전을 견주면
바로 알 수 있습니다.

빌드한 파일이 최신인지 먼저 확인하려면, 복사하기 전에 다시 빌드하세요.

```powershell
cd obsidian-plugin
npm run build            # 항상 다시 빌드한 뒤 복사합니다
```

설치된 파일에 특정 기능이 들어 있는지는 직접 찾아볼 수도 있습니다.

```powershell
# 문서 우클릭 메뉴가 들어 있는지
Select-String -Path "$pluginDir\main.js" -Pattern "growFromContext" -Quiet
```

`True` 가 나오면 그 기능이 들어 있는 빌드입니다.

> **패널은 열려 있는 동안 다시 그리지 않습니다.** 새 버전을 넣은 뒤에도 도구 절에
> 버튼이 없다면, 패널을 닫았다 다시 열어 보세요.
