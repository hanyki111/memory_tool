...
| 섹션 | 내용 |
| :--- | :--- |
| **캡처** | 항상 떠 있는 입력창. `Enter` 기록, `Shift+Enter` 줄바꿈 |
| **오늘 타임라인** | 방금 기록한 항목이 바로 아래 쌓입니다 |
| **모듈** | 필터 입력 → 클릭 한 번으로 이동 |
...
- 파일명 방식(`14.md` / `2026-08-14.md`)은 `config.yaml` 의 `timeline.filename` 을
  읽어 따릅니다. 이미 그 날짜 파일이 있으면 **어떤 방식이든 그 파일에 이어 붙입니다** —
