...
| `Ctrl+P` → `모듈 한 단계 키우기` | 🌱 다섯 단계 사다리를 한 칸 올린다 (`mmodule grow`) |
| 폴더 우클릭 → `여기에 모듈 생성` | 📁 그 경로 아래에 바로 모듈 생성 |
| 문서 우클릭 → `모듈 한 단계 키우기` | 🌱 그 모듈을 열지 않고 바로 키운다 |
| 패널 → 도구 → `모듈 한 단계 키우기` | 🌱 사이드바에서 바로 |
...
| **도구** | 컨텍스트 생성 / 경로 점검 / **모듈 한 단계 키우기** / 인덱스 동기화 |
...
### 모바일에서 시작해 PC 에서 마무리하기

모듈 생성은 파이썬이 필요하지만, **초안을 손으로 써 두고 나중에 PC 에서 키우는 것**은
됩니다. 모바일에서는 템플릿 플러그인이나 복사로 초안 문서를 만들고, 동기화한 뒤
데스크톱에서 `Ctrl+P` → **모듈 한 단계 키우기** 를 실행하면 됩니다.

지켜야 할 것은 두 가지뿐입니다.

- **파일 이름을 폴더 이름과 같게** 둡니다. `modules/asyncio/asyncio.md` 여야 하고,
  `modules/asyncio/초안.md` 로 두면 `asyncio/초안` 이라는 별개 모듈이 됩니다.
- **`**Kind:**` 줄을 남깁니다.** 어느 골격으로 키울지 정하는 근거입니다.

두 번째를 빠뜨리거나 파일 이름이 어긋나면 `mmodule grow` 가 무엇이 잘못됐는지
짚어 줍니다.
...
담고 다음에 채울 항목을 문서 끝에 순서대로 적어 둡니다.
...
결과물은 여섯 문서(`module`/`current`/`decisions`/`dependencies`/`interface`/`scope`)를
`---` 로 이어 붙인 완성된 단일 파일이고, 고른 Nature 의 목차가 `## 2. 본문` 자리에
들어갑니다. 본문이 먼저 오고 범위·전제는 문서 끝에 붙습니다.

- 템플릿을 고치려면 `<기반폴더>/templates/<kind>.md` 에 두면 번들 기본값보다 우선합니다
...
파일 탐색기에서 `modules/` 안의 폴더를 우클릭하면 **여기에 모듈 생성** 이 나옵니다.
그 폴더의 경로가 미리 채워지므로 마지막 이름만 입력하면 됩니다. 손으로 상위 경로를
적는 것이 모듈 생성에서 가장 번거롭고 틀리기 쉬운 부분인데, 파일 탐색기는 그 경로를
이미 알고 있습니다.

`modules/` 밖의 폴더와 `archive/` 에는 메뉴가 나오지 않습니다. 모듈이 그곳에 있을 수
없으므로, 항목을 띄워 봐야 나중에 오류만 납니다.

### 한 단계씩 키우기

세 곳에서 부를 수 있고, 셋 다 같은 동작입니다.

| 어디서 | 무엇을 키우나 |
| :--- | :--- |
| `Ctrl+P` → **모듈 한 단계 키우기** | 지금 열어 둔 문서 |
| 파일 탐색기에서 **문서 우클릭** | 우클릭한 그 문서 (열지 않아도 됩니다) |
| 패널 → **도구** 절의 버튼 | 지금 열어 둔 문서 |

기본 대상이 **열어 둔 문서**인 이유는, 씨앗이 좁다고 느끼는 순간이 곧 그 문서를 보고
있을 때이기 때문입니다. 그 앞에 모듈을 고르는 단계를 두면 헛걸음이 됩니다. 열린 문서가
모듈이 아니면 그때 모듈 목록을 띄웁니다.

문서 우클릭은 `modules/` 안의 모듈 문서에만 나옵니다. 타임라인이나 다른 노트에는
나오지 않습니다.

다섯 단계입니다. 씨앗 → 정체 → 뼈대 → 결론 → 완성. 네 칸이 본문 안에서 움직이고,
결정·의존성·인터페이스·범위는 마지막 한 칸에 함께 붙습니다. 실행하면 어느 칸에서
어느 칸으로 옮겨졌는지 알려 줍니다.

이미 쓴 내용은 건드리지 않습니다. 마지막 칸에 이미 있으면 아무것도 쓰지 않고 그렇다고
알려 줍니다. 한 번에 끝까지 가려면 데스크톱에서 `mmodule grow <경로> --all` 을 씁니다.
